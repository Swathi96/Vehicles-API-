...
public class CarControllerTest {
	...
	public void listCars() throws Exception {
		...
		.andExpect(MockMvcResultMatchers.jsonPath("$._embedded.carList[0].details.model",is(car.getDetails().getModel())));
	...
	public void findCar() throws Exception {
		...
		Car car = getCar();
		car.setId(1L);
		mvc.perform(get(new URI("/cars/1"))).andExpect(status().isOk()).andExpect((content().json("{'id':1}")))
	...
	@Test
	public void updateCar() throws URISyntaxException, Exception {

		Car car = new Car();
		car.setId(1l);
		car.setLocation(new Location(40.730610, -73.935242));
		Details details = new Details();
		Manufacturer manufacturer = new Manufacturer(102, "BMW");
		details.setManufacturer(manufacturer);
		details.setModel("A300");
		details.setMileage(32280);
		details.setExternalColor("white");
		details.setBody("sedan");
		details.setEngine("3.6L V6");
		...
		mvc.perform(put(new URI("/cars/"+car.getId())).content(json.write(car).getJson())
				.contentType(MediaType.APPLICATION_JSON_UTF8).accept(MediaType.APPLICATION_JSON_UTF8))
			.andExpect(MockMvcResultMatchers.jsonPath("$.id").value(car.getId()))
				.andExpect(MockMvcResultMatchers.jsonPath("$.details.model").value("A300"));
	}
}